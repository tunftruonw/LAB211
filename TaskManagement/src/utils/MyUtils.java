...
public class MyUtils {
    ...
    public static double getDouble(String mess, String errorNumberFormat, String errorOutOfRange, double min, double max) {
        while (true) {
            double ret = Double.parseDouble(getStringByRegex(mess, errorNumberFormat, "(\\d+\\.[0,5]?)"));
            if (ret < min || ret > max) {
                System.err.println(errorOutOfRange);
            } else {
                return ret;
    ...
    public static String getInputTaskType() {
        while (true) {
            String type = null;
            ...
            int choice = getInt("Enter task type: ", "Integer only", "Taks type must in range 1-4", 1, 4);
            switch (choice) {
                case 1:
                    return type = "Code";
                case 2:
